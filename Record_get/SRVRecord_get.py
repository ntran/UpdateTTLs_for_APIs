...
def SRVRecord_get(old_ttl, rec_id, zone, fqdn, token):
	...
	if ((old_ttl > 0) and (old_ttl == result['data']['ttl'])) or (old_ttl <= 0):
	    print fqdn, '-', result['data']['record_type'], '-', result['data']['rdata']['port'], result['data']['rdata']['priority'], result['data']['rdata']['target'], result['data']['rdata']['weight']
